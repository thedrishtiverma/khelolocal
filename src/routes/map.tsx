    ...
    <Page className="py-6 sm:py-10">
      <section className="india-map-story overflow-hidden rounded-2xl border border-border">
        <div className="india-map-copy">
          <p className="india-map-kicker"><span className="live-dot" /> KheloLocal / India</p>
          <h1 className="font-display text-4xl font-black uppercase leading-[0.88] sm:text-6xl">A country of games.<br /><span className="text-lime">One home ground.</span></h1>
          <p className="mt-5 max-w-md text-sm leading-6 text-surface-foreground/70 sm:text-base">Sport is already happening everywhere. KheloLocal starts by connecting one city — then the neighbourhoods inside it.</p>
          <div className="india-map-steps mt-7" aria-label="Map journey">
            {(["india", "indore", "areas"] as MapStage[]).map((item, index) => (
              <button key={item} type="button" onClick={() => setStage(item)} aria-pressed={stage === item} className={stage === item ? "active" : ""}><span>0{index + 1}</span>{item === "india" ? "India" : item === "indore" ? "Indore" : "Areas"}</button>
            ))}
          </div>
        </div>
        <div className="india-map-viewport" data-stage={stage}>
          <div className="india-map-status"><span className="live-dot" /> {stage === "india" ? "India / overview" : stage === "indore" ? "Madhya Pradesh / Indore" : "Indore / local areas"}</div>
          <svg className="india-map-svg" viewBox={INDIA.viewBox} role="img" aria-label="Map of India, focusing on Indore in Madhya Pradesh">
            <g className="india-map-shape">
              {INDIA.locations.map((location) => <path key={location.id} d={location.path} className={location.id === "mp" ? "india-state indore-state" : "india-state"} />)}
              <g className="indore-marker"><circle cx="244" cy="323" r="5" /><circle className="indore-marker-pulse" cx="244" cy="323" r="10" /><text x="256" y="317">INDORE</text></g>
              <g className="india-area-markers">{AREA_MARKERS.map((area) => <g key={area.id} className={zoneId === area.id ? "active" : ""} onClick={() => { setZoneId(area.id); setStage("areas"); }} role="button" tabIndex={0}><circle cx={area.x} cy={area.y} r="3.5" /><text x={area.x + 7} y={area.y - 5}>{area.label}</text></g>)}</g>
            </g>
          </svg>
          <p className="india-map-caption">{stage === "india" ? "India first. The local network begins with a single point." : stage === "indore" ? "Zooming in on Indore, Madhya Pradesh." : "Choose an area to explore what is happening nearby."}</p>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <section className="india-map-detail rounded-2xl border border-border p-6 sm:p-8">
          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-lime">From country to street</p>
          <h2 className="mt-4 font-display text-3xl font-black uppercase">The local game, made visible.</h2>
          <p className="mt-4 max-w-2xl text-sm leading-6 text-muted-foreground">The map moves from the national picture to Indore, then into the places where players actually meet, train and compete.</p>
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            {[
              [Trophy, db.tournaments.length, "Tournaments"],
              [Building2, db.fieldSubmissions.filter((r) => r.status === "VERIFIED").length, "Verified places"],
              [Users, db.athletes.length, "Athletes"],
            ].map(([Icon, value, label]) => { const StatIcon = Icon as typeof Trophy; return <div key={label as string} className="border-l border-border pl-4"><StatIcon className="size-5 text-lime" /><p className="stat-num mt-4 text-3xl">{value as number}</p><p className="mt-1 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{label as string}</p></div>; })}
          </div>
        </section>
          ...
          <section className="data-card rounded-2xl p-6 sm:p-8">
            ...
            <p className="mt-4 text-xs leading-5 text-muted-foreground">Select an area on the map to see verified sports activity nearby.</p>
