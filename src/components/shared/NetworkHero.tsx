...
export function NetworkHero({
  ...
  const isHomeHero = className?.includes("home-network-hero");
  ...
  aria-hidden="true"
  style={isHomeHero ? { display: "none" } : undefined}
/>

<div
  className="network-hero-orbit network-hero-orbit-one"
  aria-hidden="true"
  style={isHomeHero ? { display: "none" } : undefined}
/>

<div
  className="network-hero-orbit network-hero-orbit-two"
  aria-hidden="true"
  style={isHomeHero ? { display: "none" } : undefined}
/>
      {/* HERO CONTENT */}
      <div className="relative mx-auto grid w-full max-w-6xl gap-10 px-4 py-14 sm:px-6 sm:py-20 lg:grid-cols-[1.15fr_0.85fr] lg:items-end lg:gap-16 lg:py-24">
        ...
        <div className="relative z-10">
          <p className="network-hero-eyebrow">
            {eyebrow}
          </p>
        ...
        <div className="network-hero-console relative z-10 hidden overflow-hidden rounded-2xl p-5 sm:p-6 lg:block">
          ...
          <div className="relative mt-8 grid grid-cols-3 gap-3">
            {highlights.map((item, index) => (
              <div
                key={item}
                className="network-hero-node"
              >
                <span className="font-num text-xs text-lime">
                  0{index + 1}
                </span>
